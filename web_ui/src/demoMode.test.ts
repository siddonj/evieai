...
const demoMode = await import('./demoMode.ts')
...
test('defines a product-demo scenario for multi-source chat with export', () => {
  ...
  assert.deepEqual(DEMO_SCENARIOS[0].prompts, [
...
test('advances through the guided sequence one step at a time', () => {
  assert.equal(getNextDemoStep(0), 1)
  assert.equal(getNextDemoStep(1), 2)
  assert.equal(getNextDemoStep(2), 2)
})
